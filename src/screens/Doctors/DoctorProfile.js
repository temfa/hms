...
import PreviewDoctor from "../../components/Preview/previewDoctor";
import LoadingSkel from "../../components/LoadingSkel";
import { useGetSingleUsers } from "../../hooks/useGetSingleUser";
...
function DoctorProfile() {
  ...
  const { singleUsers, getSingleUserLoading } = useGetSingleUsers(locate[locate.length - 1], "/doctors");
  ...
  const tabPanel = () => {
    switch (activeTab) {
      case 1:
        return <PreviewDoctor data={singleUsers} />;
      // case 2:
      //   return <PatientsUsed />;
      case 3:
      ...
      {getSingleUserLoading ? (
        ...
        <>
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold">Dr. {singleUsers?.fullname}</h1>
          </div>
          <div className=" grid grid-cols-12 gap-6 my-8 items-start">
            <div
              ...
              <img src="/images/doctoravatar.png" alt="setting" className="w-40 h-40 rounded-full object-cover border border-dashed border-subMain" />
              <div className="gap-2 flex-colo">
                <h2 className="text-sm font-semibold">Dr. {singleUsers?.fullname}</h2>
                <p className="text-xs text-textGray">{singleUsers?.email}</p>
                <p className="text-xs">{singleUsers?.contact_number}</p>
              </div>
              {/* tabs */}
