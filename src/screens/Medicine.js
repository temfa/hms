...
import { useGetAllMedicine } from "../hooks/useGetAllMedicine";
import LoadingSkel from "../components/LoadingSkel";

const Medicine = () => {
  const { allMedicines, getAllMedicineLoading } = useGetAllMedicine();
      ...
      <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="100" data-aos-offset="200" className="bg-white my-8 rounded-xl border-[1px] border-border p-5">
        ...
        <div className="grid md:grid-cols-6 grid-cols-1 gap-2">
          <div className="md:col-span-5 grid lg:grid-cols-4 xs:grid-cols-2 items-center gap-2">
            <input type="text" placeholder='Search "paracetamol"' className="h-14 w-full text-sm text-main rounded-md bg-dry border border-border px-4" />
        ...
        {getAllMedicineLoading ? (
          <LoadingSkel />
        ) : (
          <div className="mt-8 w-full overflow-x-scroll">
            <MedicineTable data={allMedicines} onEdit={onEdit} />
          </div>
        )}
      </div>
    </Layout>
