...
function Sidebar() {
  ...
  return (
    <div className="bg-white xl:shadow-lg py-6 px-4 xl:h-screen w-full border-r border-border">
      <Link to="/">
        <img src="/images/logo.png" alt="logo" className="w-2/4 h-12 ml-4 object-contain" />
      </Link>
      <div className="flex-colo gap-2 mt-12">
        {MenuDatas().map((item, index) => (
          <Link
            ...
            `}
            />
            <p className={`text-sm font-medium group-hover:text-subMain ${currentPath(item.path) === item.path ? "text-subMain" : "text-gray-500"}`}>{item.title}</p>
          </Link>
        ))}
      </div>
    </div>
