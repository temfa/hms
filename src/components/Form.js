  ...
  return (
    <div className="text-sm w-full">
      ...
      <input
        ...
        className={`w-full bg-transparent text-sm mt-3 p-4 border ${color ? "border-border font-light" : "border-white text-white"} rounded-lg focus:border focus:border-subMain`}
      />
    </div>
  );
};
...
export const Select2 = ({ children, selectedPerson, setSelectedPerson, datas, test }) => {
  return (
    <div className="text-sm relative w-full ">
      <div className="w-full">
        <Listbox value={selectedPerson} onChange={setSelectedPerson}>
          ...
          <Listbox.Options className="flex  flex-col gap-4 top-10 z-50 absolute left-0 w-full bg-white rounded-md shadow-lg py-4 px-6 ring-1 ring-border focus:outline-none">
            {datas?.map((person, index) => (
              <Listbox.Option className={`cursor-pointer text-xs hover:text-subMain`} key={index} value={person} disabled={person.unavailable}>
                {test ? person.bed_no : person.fullname}
              </Listbox.Option>
            ))}
          </Listbox.Options>
        </Listbox>
  ...
  return (
    <div className="text-sm w-full flex flex-row items-center">
      ...
      <label className="flex-colo cursor-pointer relative">
        <input type="checkbox" name={name} checked={checked} onChange={onChange} className="absolute opacity-0 w-0 h-0" />
        <span className={` border rounded  w-5 h-5 flex flex-shrink-0 justify-center items-center mr-2 ${checked ? "border-subMain bg-subMain" : "border-gray-300 bg-white"}`}>
