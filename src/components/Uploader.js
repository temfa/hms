...
const Uploader = ({ setImage, image }) => {
  ...
  return (
    <div className="w-full text-center grid grid-cols-12 gap-4">
      <div className="px-6 lg:col-span-10 sm:col-span-8 col-span-12 pt-5 pb-6 border-2 border-dashed rounded-md cursor-pointer" {...getRootProps()}>
        <input {...getInputProps()} />
      ...
      <div className="lg:col-span-2 sm:col-span-4 col-span-12">
        {loading ? (
          ...
          <img src={image ? image : "http://placehold.it/300x300"} alt="preview" className=" w-full h-32 rounded object-cover" />
        )}
      </div>
    </div>
  );
